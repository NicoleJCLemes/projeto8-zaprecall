...
export default function Card(props){
    ...
    if(content === 'questionName'){
        ...
        return(
            <CardAnswer color={props.color} answer={props.answer} variable={props.variable} callbackColor={((update) => props.callbackColor(update))} callback={((update) => setContent(update))} callbackAnswered={(update) => props.callback(update)} />
        )
    } else if (content === 'questionResult') {
        return(
            <CardResult name={props.name} color={props.color} callbackColor={((update) => props.callbackColor(update))} />
    )
    }
